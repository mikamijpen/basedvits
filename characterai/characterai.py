...
def goto(link: str, *, wait: bool = True, token: str = None):
    if token != None:
    ...
    if page.title() != 'Waiting Room powered by Cloudflare':
        ...
        if wait:
            ...
            raise errors.Loaded('The Site is Overloaded', 10)  # 抛出错误



def GetResponse(
        link: str, *, wait: bool = True,
        token: str = None
):
...
class PyCAI:
    def __init__(self, token: str, *, headless: bool = True):
        ...
        self.context.set_default_timeout(90000) # 90s
    ...
    class user:
        ...
        def followers(self, *, wait: bool = True, token: str = None):
            return GetResponse(
                'chat/user/followers/',
                wait=wait, token=token
            )

        def following(self, *, wait: bool = True, token: str = None):
            return GetResponse(
                'chat/user/following/',
                wait=wait, token=token
            )

        def recent(self, *, wait: bool = True, token: str = None):
            return GetResponse(
    ...
    class chat:
        def rate(
            ...
            if rate == 0:
                label = [234, 238, 241, 244]  # Terrible
            elif rate == 1:
                label = [235, 237, 241, 244]  # Bad
            elif rate == 2:
                label = [235, 238, 240, 244]  # Good
            elif rate == 3:
                label = [235, 238, 241, 243]  # Fantastic
            else:
                raise errors.LabelError('Wrong Rate Value')
